...
void tree_insert_sem(sem_lock_t *lock, int value, BST_t *bst)
{
...
void tree_insert(int value, BST_t *bst)
{
...
void deleteValSem(sem_lock_t *lock, int value, BST_t *bst)
{
    // acquire lock
    sem_wait(&lock->criticalLock);
    sem_wait(&lock->treeLock);
...
void deleteVal(int value, BST_t *bst)
{
    node *current = bst->head;
    if (current->data == value)
    {
...
void printTreeInorderSem(node *head, sem_lock_t *lock)
{
...
{
    printf("-------------delete-node----------------\n");
    deleteValSem(lock, rand() % MAX_VALUE, bst);
    printTreeInorderSem(bst->head, lock);
    printf("\n");
}
...
void handleThreads(int num_threads, sem_lock_t lock, BST_t bst)
{
